...
import static io.keikai.model.SCell.CellType.FORMULA;
import static io.keikai.model.SCell.CellType.STRING;
...
public class RangeImpl implements SRange, Serializable {
	...
	public void setEditText(final String editText) {
		...
		final ResultWrap<SHyperlink.HyperlinkType> hyperlinkType = new ResultWrap<SHyperlink.HyperlinkType>();
		new CellVisitorTask(new CellVisitorForUpdate() {
			public boolean visit(SCell cell) {
				...
				if((result = input.get())==null){
					...
					if(result.getType() == STRING){
						final SHyperlink.HyperlinkType type = getHyperlinkType((String)result.getValue());
						if (type == SHyperlink.HyperlinkType.EMAIL || type == SHyperlink.HyperlinkType.URL) {
	...
	private SHyperlink setupHyperlink0(SCell cell, SHyperlink.HyperlinkType type,String address,String label) {
		SHyperlink.HyperlinkType type0 = getHyperlinkType(address);
		if (type != SHyperlink.HyperlinkType.DOCUMENT || type0 != SHyperlink.HyperlinkType.FILE) {
			if (type0 != type) {
				switch(type) {
				case DOCUMENT:
...
//		FormulaCacheCleaner cacheCleaner = new FormulaCacheCleaner(bookSeries);
					...
					for(final String var:variables){
						Set<Ref> precedents = table.searchPrecedents(new DependencyTable.RefFilter() {
							...
							public boolean accept(Ref ref) {
								...
								if(ref.getBookName().equals(bookName) && ref.getType()== Ref.RefType.NAME){
	...
	@Override
	public void setBorders(final ApplyBorderType borderType,final SBorder.BorderType lineStyle,
			final String color) {
		new ModelManipulationTask(){
			@Override
			protected Object doInvoke() {
	...
	@Override
	public void setHyperlink(final SHyperlink.HyperlinkType linkType,final String address,
...
//				final int newIndex =  index < (sheetCount - 1) ? index : (index - 1);
	...
	@Override
	public SAutoFilter enableAutoFilter(final int field, final SAutoFilter.FilterOp filterOp,
	...
	@Override
	public SChart addChart(final ViewAnchor anchor, final SChart.ChartType type,
			final SChart.ChartGrouping grouping, final SChart.ChartLegendPosition pos, final boolean isThreeD) {
	...
	@Override
	public void setValidation(final SDataValidation.ValidationType validationType,
			final boolean ignoreBlank, final SDataValidation.OperatorType operatorType,
			...
			final boolean showError, final SDataValidation.AlertStyle alertStyle, final String errorTitle,
	...
	private SDataValidation setValidaitonInLock(
...
//		}
	...
	private boolean isDefaultValidationParameter(
			SDataValidation.ValidationType validationType,
			boolean ignoreBlank, SDataValidation.OperatorType operatorType,
			boolean inCellDropDown, String formula1, String formula2,
			boolean showInput, String inputTitle, String inputMessage,
			boolean showError, SDataValidation.AlertStyle alertStyle, String errorTitle,
			String errorMessage) {
		return validationType == SDataValidation.ValidationType.ANY && showInput && inputTitle.isEmpty() && inputMessage.isEmpty() &&
				showError && alertStyle == SDataValidation.AlertStyle.STOP && errorTitle.isEmpty() && errorMessage.isEmpty();
	}
