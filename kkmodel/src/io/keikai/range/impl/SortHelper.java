...
public class SortHelper extends RangeHelperBase {
	 ...
	 */
	public void sort(SRange key1, boolean descending1, SRange.SortDataOption dataOption1, SRange key2, boolean descending2, SRange.SortDataOption dataOption2, SRange key3,
			boolean descending3, SRange.SortDataOption dataOption3, int header, boolean matchCase, boolean sortByRows) {
		...
		final SRange.SortDataOption[] dataOptions = new SRange.SortDataOption[keyCount];
	...
	private List<SortKey> collectKeys(boolean sortByRows, int keyCount,
			final SRange.SortDataOption[] dataOptions, final int[] keyIndexes) {
	...
	private void pasteToNewCell(PasteCellHelper helper, CellBuffer buffer, 
		...
		if(buffer.isNull()){
			...
			helper.pasteValue(buffer,cell,cutFrom,
					buffer.getType() == SCell.CellType.FORMULA,rowOffset,
