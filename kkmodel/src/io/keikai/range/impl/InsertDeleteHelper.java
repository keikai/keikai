...
public class InsertDeleteHelper extends RangeHelperBase {
	...
	private STable deleteTable(SRange.DeleteShift shift, Set<String> toShift) {
		...
		if (overlap != null) {
			...
			collectAndCheckShiftTables(row1, col1, row2, col2, toShift, shift == SRange.DeleteShift.LEFT /*horizontal*/);
		}
	...
	public void delete(SRange.DeleteShift shift) {
		...
		if(isWholeRow()) { // ignore insert direction
			...
			//ZSS-985
//			final Set<String> toShift = new HashSet<String>();
			overlap = deleteTable(SRange.DeleteShift.UP, toShift);
			if (overlap != null) {
			...
			//ZSS-985
//			final Set<String> toShift = new HashSet<String>();
			overlap = deleteTable(SRange.DeleteShift.LEFT, toShift);
			deleteCols(overlap);
		...
		} else if(shift != SRange.DeleteShift.DEFAULT) { // do nothing if "DEFAULT", it's according to XRange.delete() spec.
			//ZSS-985
//			final Set<String> toShift = new HashSet<String>();
			...
			if (overlap != null) {
				...
				sheet.deleteCell(row1, col1, row2, col2, shift == SRange.DeleteShift.LEFT);
			}
		}
		//ZSS-988
		if (overlap != null) {
	...
	private STable insertTable(SRange.InsertShift shift, Set<String> toShift) {
		final Set<String> overlapped = new HashSet<String>(2);
		String message = getInsertedTables(range, overlapped, shift == SRange.InsertShift.RIGHT); //ZSS-986
		if (message != null)
	...
	public void insert(
			SRange.InsertShift shift, SRange.InsertCopyOrigin copyOrigin) {
		...
		if(isWholeRow()) { // ignore insert direction
			...
			//ZSS-986
//			final Set<String> toShift = new HashSet<String>();
			overlap = insertTable(SRange.InsertShift.DOWN, toShift);
			insertTableRows(overlap);
			shiftTables(book, toShift, row2 - row1 + 1, false /*horizontal*/);
			
			if (overlap != null) {
				copyOrigin = SRange.InsertCopyOrigin.FORMAT_LEFT_ABOVE; //always copy left/above style in Table case
			}
			...
			} else if(copyOrigin == SRange.InsertCopyOrigin.FORMAT_RIGHT_BELOW) {
			...
			//ZSS-986
//			final Set<String> toShift = new HashSet<String>();
			overlap = insertTable(SRange.InsertShift.RIGHT, toShift);
			insertTableCols(overlap, copyOrigin == SRange.InsertCopyOrigin.FORMAT_LEFT_ABOVE);
			shiftTables(book, toShift, col2 - col1 + 1, true /*horizontal*/);
		...
		} else if(shift != SRange.InsertShift.DEFAULT) { // do nothing if "DEFAULT", it's according to XRange.insert() spec.
			//ZSS-986
//			final Set<String> toShift = new HashSet<String>();
			...
			if (overlap != null) {
				...
				if (shift == SRange.InsertShift.RIGHT) {
					...
					if (col1 == col2 && isLastColumn
						&& copyOrigin == SRange.InsertCopyOrigin.FORMAT_RIGHT_BELOW) { //Insert Table Column to the Right
						++col1;
						++col2;
					}
			...
			sheet.insertCell(row1, col1, row2, col2, shift == SRange.InsertShift.RIGHT);
			...
			// copy style/formal/size
			if(shift == SRange.InsertShift.RIGHT) { // horizontal
				if(copyOrigin == SRange.InsertCopyOrigin.FORMAT_LEFT_ABOVE) {
					if(col1 - 1 >= 0) {
