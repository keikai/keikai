...
public class CellMouseCommand extends AbstractCommand implements Command {
	...
	public void process(AuRequest request) {
		...
		if ("lc".equals(type)) {
			type = io.keikai.ui.event.Events.ON_CELL_CLICK;
		} else if ("rc".equals(type)) {
			type = io.keikai.ui.event.Events.ON_CELL_RIGHT_CLICK;
		} else if ("dbc".equals(type)) {
			type = io.keikai.ui.event.Events.ON_CELL_DOUBLE_CLICK;
		} else if ("af".equals(type)) {
			type = io.keikai.ui.event.Events.ON_CELL_FILTER;
		} else if ("dv".equals(type)) {
			type = io.keikai.ui.event.Events.ON_CELL_VALIDATOR;
		} else {
			throw new UiException("unknow type : " + type);
		}

		if (io.keikai.ui.event.Events.ON_CELL_FILTER.equals(type)) {
		...
		} else if (io.keikai.ui.event.Events.ON_CELL_VALIDATOR.equals(type)) {
